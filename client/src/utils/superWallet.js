 ...
 */
export async function bridgeTokenFrom1To2Private(
  toAddress,
  tokenAmount,
  tokenAddress
) {
  console.log(toAddress, tokenAmount, tokenAddress);
  const abi = [
  ...
  return `${alpha1rpcblockscout}/tx/${tx.hash}`;
}

export async function totalTokenBalancePrivate(tokenAddress) {
  const rpcArray = [alpha1rpc, alpha2rpc];
...
export async function totalEthBalancePrivate() {
  const rpcArray = [alpha1rpc, alpha2rpc];

  const signer = new ethers.Wallet(process.env.PRIVATE_KEY, provider1);
  const walletAddress = signer.address;
  let balance = 0n;
  for (let i = 0; i < rpcArray.length; i++) {
    const provider = new ethers.JsonRpcProvider(rpcArray[i]);
